...
case 'files':
	...
	if(in_array($newTarget[1], $EXTENSIONS)){
		...
		if (copy(realpath(UPLOAD_TMP_DIR. DIRECTORY_SEPARATOR. basename($target)),realpath(VB_FILE_PATH. DIRECTORY_SEPARATOR .basename($file)))) {
			$sql = "INSERT INTO adjuntossub(idsubcluster,titulo,archivo,create_user) VALUES($id,'$name','$file',$appuser->uid)";
...
case 'obs':
	...
	$estado = getSQLValue("SELECT idestadosub FROM subcluster WHERE id=$id");
	$user = getAppUser();
	$sql = "INSERT INTO seguimientosub(idsubcluster,idestadosub,idusuario,notas)
	VALUES ($id,$estado,$user->uid,$obs)";
	if (db_query($sql,true) > 0){
				db_query("UPDATE `subcluster` SET modify_user=$user->uid,modify_date=CURRENT_TIMESTAMP WHERE id=$id",true);
		echo "OK";
	}
	else echo "No fue posible adicionar la observacion.";
	break;
 ...
 case 'cancelar':
	...
	$sql = "UPDATE `subcluster` SET idestadosub=$SUB_ST_CANCELADA,notas=$obs,modify_user=$user->uid,modify_date=CURRENT_TIMESTAMP WHERE id=$id";
	if (db_query($sql,true) > 0){


        db_query(   "INSERT INTO seguimientosub(idsubcluster,idestadosub,idusuario,notas)
	VALUES ($id,$estado,$user->uid,$obs)");
