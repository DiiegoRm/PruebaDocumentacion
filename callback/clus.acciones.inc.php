...
switch($_REQUEST["mode"]){
 case 'cambio':
	...
	if(in_array($newTarget[1], $EXTENSIONS)){
		...
		if (copy(realpath(UPLOAD_TMP_DIR. DIRECTORY_SEPARATOR. basename($target)),realpath(VB_FILE_PATH. DIRECTORY_SEPARATOR .basename($file)))) {
			$sql = "INSERT INTO adjuntosclus(idcluster,titulo,archivo,create_user) VALUES($id,'$name','$file',$appuser->uid)";
 ...
 case 'AproObservacion':
	$id = getVal($_POST['id'],"null");
	$Aprobacion = getVal($_POST['Apro'],"null");
	$user = getAppUser();
...
case 'obs':
	...
	$estado = getSQLValue("SELECT idestadoclus FROM clusters WHERE id=$id");
	$user = getAppUser();
	$sql = "INSERT INTO seguimientoclus(idcluster,idestadoclus,idusuario,notas)
	VALUES ($id,$estado,$user->uid,$obs)";
	if (db_query($sql,true) > 0){
				db_query("UPDATE `clusters` SET modify_user=$user->uid,modify_date=CURRENT_TIMESTAMP WHERE id=$id",true);
		echo "OK";
	}
	else echo "No fue posible adicionar la observacion.";
	break;
 ...
 case 'cancelar':
	...
	$sql = "UPDATE `clusters` SET idestadoclus=$CLUS_ST_CANCELADA,notas=$obs,modify_user=$user->uid,modify_date=CURRENT_TIMESTAMP WHERE id=$id";
	if (db_query($sql,true) > 0){


        db_query(    "INSERT INTO seguimientoclus(idcluster,idestadoclus,idusuario,notas)
	VALUES ($id,$estado,$user->uid,$obs)");
...
case 'files':
	...
	if(in_array($newTarget[1], $EXTENSIONS)){
		...
		if (copy(realpath(UPLOAD_TMP_DIR. DIRECTORY_SEPARATOR. basename($target)),basename(VB_FILE_PATH. DIRECTORY_SEPARATOR .basename($file)))) {
			$sql = "INSERT INTO adjuntosclus(idcluster,titulo,archivo,create_user) VALUES($id,'$name','$file',$appuser->uid)";
			$sql_update = db_query($sql,true);
			unlink(realpath(UPLOAD_TMP_DIR. DIRECTORY_SEPARATOR .basename($target)));
			echo "OK";
 ...
 case 'retornar':
    ...
    db_query("UPDATE reprogramacion SET idestadoot=$estado,fecha_requerida_nueva=$requerida,modify_date=CURRENT_TIMESTAMP,idusuario=$user->uid WHERE id=$idrepro and idorden=$id and fecha_requerida_nueva='0000-00-00'",true);
