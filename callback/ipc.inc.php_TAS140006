...
switch($_REQUEST["mode"]) {
    ...
    case 'process':
        ...
        $errors = Array();
        $success = Array();
        $file = fopen(LOG_FILE_PATH . DIRECTORY_SEPARATOR . "imp_ipc.log", "w");
		for($row = 2; $row <= $lastRow; $row++) {             
            $cellA = $worksheet->getCell('A'.$row)->getValue();
            $cellB = $worksheet->getCell('B'.$row)->getValue();
            $cellB = PHPExcel_Shared_Date::ExcelToPHP($cellB);
            $cellB = gmdate('Y-m-d', $cellB);
            $cellC = $worksheet->getCell('C'.$row)->getValue();
            $cellC = PHPExcel_Shared_Date::ExcelToPHP($cellC);
            $cellC = gmdate('Y-m-d', $cellC);
            $cellD = $worksheet->getCell('D'.$row)->getValue();

                if(($cellA != '') && ($cellB != '') && ($cellC != '') && ($cellD != '')) {

                    // Validar que este para ese contrato por fuera del rango de fechas

                    
                            $sql_update = db_query("INSERT INTO `ipc` (`value`,`start_date`,`end_date`,`idcontrato`,`create_date`) VALUES ($cellA, '". $cellB."', '".$cellC."', $cellD, NOW())");
                            $message = $row . '| Success - Se ha creado esta relación satisfactoriamente (value = '. $cellA .', start_date = '. $cellB .', 
                                end_date = '. $cellC .', idcontrato = '. $cellD .').';
                            array_push($success, $message);
                } else {
                    $message = $row . '| Error - Uno de los valores o ambos está vacio (value = '. $cellA .', start_date = '. $cellB .', end_date = '. $cellC .', 
                        idcontrato = '. $cellD .').';
                    array_push($errors, $row . '| Error - Uno de los valores o ambos está vacio (value = '. $cellA .', start_date = '. $cellB .', 
                    end_date = '. $cellC .', idcontrato = '. $cellD .').');
                }
