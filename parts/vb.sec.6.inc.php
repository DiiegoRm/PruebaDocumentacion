<table id="seg-vb-ro" class="ui-widget ui-widget-content" style="width: 100%">
	...
	$query = db_query("SELECT s.start,s.finish,s.secs,s.notas,u.nombre usuario,e.nombre estado FROM (
			SELECT  t1.ID, t1.idestadovb, t1.idusuario,t1.notas, t1.create_date start, IFNULL(t2.create_date, 'Fecha Actual') finish,
			IFNULL(UNIX_TIMESTAMP(t2.create_date), UNIX_TIMESTAMP(CURRENT_TIMESTAMP)) - UNIX_TIMESTAMP(t1.create_date) AS secs
			FROM seguimientovb t1
			LEFT JOIN seguimientovb t2
			  ON t2.create_date > t1.create_date AND
				t2.ID <> t1.ID AND
				t2.idviabilidad = t1.idviabilidad
			LEFT JOIN seguimientovb t3
			  ON t3.idviabilidad = t1.idviabilidad AND
			...
			WHERE t3.ID IS NULL AND
			  t1.idviabilidad = $id
			ORDER BY t1.ID
		) s, usuarios u, estadovb e
		WHERE s.idestadovb=e.id AND s.idusuario=u.id
		ORDER BY s.start");
	$i=0;
