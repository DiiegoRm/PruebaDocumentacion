...
<table class="ui-widget ui-widget-content" style="width:100%">
	...
	//  echo "SELECT * FROM adjuntosequipos ad JOIN equipos_ecc ecc ON ecc.id = ad.idequipo WHERE ad.idusuario = $appuser->uid AND ecc.id = $data[id]";die();
		$query = db_query("SELECT ad.*, u.nombre user FROM adjuntosequipos ad JOIN equipos_ecc ecc ON ecc.id = ad.idequipo JOIN usuarios u ON u.id = ad.idusuario WHERE $where ecc.id = $data[id]");
		...
		if($count != 0){
			...
			while($row = mysqli_fetch_array($query)) {
				...
				// if($appuser->isAdmin()||$appuser->isInRole($ADMINISTRACION)){
				// 	echo "<td><span class='ui-icon ui-icon-trash' onclick='deleteFile(".htmlspecialchars($row[id]).",\"ot\")'></span></td>\n";
				// }else{ 
					echo "<td>-</td>\n";
				// }
				echo "</tr>\n";
