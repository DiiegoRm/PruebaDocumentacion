...
<table class="ui-widget ui-widget-content" style="width:100%">
	...
	while($row = mysqli_fetch_array($query)) {
		...
		echo "<td><a href=\"includes/descarga.inc.php?document=".htmlspecialchars(trim($row[archivo]))."&ruta=" . str_replace('/sgp', '', PP_FILE_WEB) . "&name=" . htmlspecialchars($row[titulo]). "\">".htmlspecialchars($row[titulo])."</a></td>\n";		if($row['create_user']==$appuser->uid)echo "<td><span class='ui-icon ui-icon-trash' onclick='deleteFile(".htmlspecialchars($row[id]).",\"pp\")'></span></td>\n";
		else echo "<td>-</td>\n";
		echo "</tr>\n";
	}
	?>
