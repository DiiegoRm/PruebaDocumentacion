...
if(isLoggedIn()){
	if(getMenu()=='9999'){
        ...
        switch(getMenu()){
			...
			break;
			case '100':case '101':
				if($appuser->isAdminreco() || $appuser->isInGroup($STC) || $appuser->isInRole($CARGAR_PRESUPUESTO)){
			...
			/*case '833':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/baremosAsociar.inc.php";
				} else {
					notAuthorized();
				}
			break;*/
			...
			break;
			case '826':
				if($appuser->isInRole($GESTIONAR_TABLAS) || $appuser->isInRole($ADMINISTRADOR_RECOBRE) ){
			...
			case '840':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_central.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '841':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_region.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '842':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_poligono.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '843':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_comunas.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '844':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_zonas.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '845':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_vb.inc.php";
				} else {
					notAuthorized();
			}
			break;
			case '846':
				if($appuser->isInRole($GESTIONAR_TABLAS)){
					include_once "modules/$folder/gestion_cluster.inc.php";
				} else {
					notAuthorized();
			}
			break;
			...
			break;
			case '1100':case '1101':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE) || $appuser->isInGroup($ALMACENISTA)||$appuser->isInGroup($CONTRATISTACOBRE)  ){
					
					include_once "modules/$folder/repliegue.add.inc.php";
				} else {
					notAuthorized();
				}
			break; 
			case '1102':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)){
			
					include_once "modules/$folder/repliegue.add2.inc.php";
				} else {
					notAuthorized();
				}
			break;
			case '1103':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)){
					...
					include_once "modules/$folder/terreno.add.inc.php";
				} else {
					notAuthorized();
				}
			break;
			...
			case '1105':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)|| $appuser->isInGroup($ALMACENISTA) || $appuser->isInGroup($CONTRATISTACOBRE)){
					if(!isset($_REQUEST['mode'])) $_REQUEST["mode"] = 'add';
					include_once "modules/$folder/token.add.inc.php";
				} else {
					notAuthorized();
				}
				break;
				case '1106':
					if($appuser->isInGroup($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($ALMACENISTA) || $appuser->isInGroup($EECC)){
					
						include_once "modules/$folder/ingreso_retal.php";
					} else {
						notAuthorized();
					}
			break;
			case '1107':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE)  ||$appuser->isInGroup($CONTRATISTACOBRE)  || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)|| $appuser->isInGroup($ALMACENISTA)){
					$vr = $appuser->isInRole($ADMINISTRADOR_RECOBRE);
					include_once "modules/$folder/contratista_cobre.php";
				} else {
					notAuthorized();
				}
		break;
		case '1108':
			if($appuser->isInRole($ADMINISTRADOR_RECOBRE)  || $appuser->isInGroup($CONTRATISTACOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)|| $appuser->isInGroup($ALMACENISTA)){
				if(!isset($_REQUEST['mode'])) $_REQUEST["mode"] = 'add';
				include_once "modules/$folder/contratista_cobre_repliegue.php";
			} else {
				notAuthorized();
			}
		break;
		case '1109':
			if($appuser->isInRole($ADMINISTRADOR_RECOBRE)  ){
				
				include_once "modules/$folder/tipo_proyecto.php";
			} else {
				notAuthorized();
			}
			break;
			case '1110':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE)  ){
					...
					include_once "modules/$folder/trmlme.php";
				} else {
					notAuthorized();
				}
				break;	
				case '1111':
					if($appuser->isInRole($ADMINISTRADOR_RECOBRE) || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE)|| $appuser->isInGroup($ALMACENISTA)){

						include_once "modules/$folder/editar_cable.php";
					} else {
						notAuthorized();
					}
					break;	
					case '1112':
						if($appuser->isInRole($ADMINISTRADOR_RECOBRE)    || $appuser->isInGroup($STC) || $appuser->isInGroup($ALMACENISTA)){
						
							include_once "modules/$folder/reutilizacion.php";
						} else {
							notAuthorized();
						}
				break;
				case '1113':
					if($appuser->isInRole($ADMINISTRADOR_RECOBRE)    || $appuser->isInGroup($STC) || $appuser->isInGroup($ALMACENISTA)){
						if(!isset($_REQUEST["mode"]))$_REQUEST["mode"] = 'buscar';
						include_once "modules/$folder/reutilizacion.php";
					} else {
						notAuthorized();
					}
			break;
			case '1114':
				if($appuser->isInRole($ADMINISTRADOR_RECOBRE)    || $appuser->isInGroup($STC) ){
					if(!isset($_REQUEST["mode"]))$_REQUEST["mode"] = 'bandeja_reuso';
					include_once "modules/$folder/reutilizacion.php";
				} else {
					notAuthorized();
				}
	...
	case '1116':
		if($appuser->isInRole($ADMINISTRADOR_RECOBRE)  ){
			if(!isset($_REQUEST["mode"]))$_REQUEST["mode"] = 'email';
			include_once "modules/$folder/trmlme.php";
		} else {
			notAuthorized();
		}
		break;	
			...
			break;
			case '9002':
				if($appuser->isInRole($ADMINISTRACION)|| $appuser->isInGroup("$ADMINISTRADOR_RECOBRE")){
			...
			break;
			case '9004':
				if($appuser->isInRole($ADMINISTRACION)|| $appuser->isInGroup("$ADMINISTRADOR_RECOBRE") || $appuser->isInGroup($STC)|| $appuser->isInGroup($EECC)|| $appuser->isInGroup($SEGURIDAD_RECOBRE) ){
