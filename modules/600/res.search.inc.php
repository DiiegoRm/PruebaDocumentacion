...
switch($_REQUEST["mode"]){
 case 'view':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT s.tipo,s.fecha,s.idestadores,e.nombre estado,s.idorden,s.create_date,s.modify_date, CONCAT(m.codigo,'|',m.item) material,s.fecha,s.cantidad FROM `reservas` s, estadores e, material m WHERE s.idestadores=e.id AND s.idmaterial=m.id AND s.`id` = '$id'");
		...
		<div class="outerbox">
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=aprove">
				...
				<div id="tabs">
					...
					<div id="tabs-2">
						<?php include_once "parts/res/tab.seguimiento.inc.php"; ?>
					</div>
				</div>
				<br class="clear"/>
...
default:
	...
	/*if($_POST['delState']){
		...
		$sql = "SELECT s.id,s.numero,s.idorden,o.numero orden, eo.nombre estadoot, ex.nombre eecc,z.nombre zona, d.nombre depto,l.nombre localidad,s.fecha,s.tipo,DATE_FORMAT(s.create_date,'%Y-%m-%d') creado, s.cantidad,e.nombre estado,s.active,m.codigo,m.item,o.pm_orden,o.pm_reserva FROM reservas s, estadores e, ordenes o,estadoot eo, contratos c,eecc ex, zonas z, deptos d,localidades l,material m WHERE s.idorden=o.id AND s.idestadores=e.id $variable AND o.idestadoot=eo.id AND o.idcontrato=c.id AND c.ideecc=ex.id AND o.idzona=z.id AND o.iddepto=d.id AND o.idlocalidad=l.id AND s.idmaterial=m.id $locationfilter".getALLSQLFilters().getSQLSort("s.create_date","DESC");
		$q = db_query($sql);
