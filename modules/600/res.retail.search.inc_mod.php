...
switch($_REQUEST["mode"]){
 case 'view':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT s.tipo,s.fecha,s.idestadores,e.nombre estado,s.idorden,s.create_date,s.modify_date, CONCAT(m.codigo,'|',m.item) material,s.fecha,s.cantidad,s.lote,s.peso FROM `reservasretal` s, estadores e, material m WHERE s.idestadores=e.id AND s.idmaterial=m.id AND s.`id` = '$id'");
	$row = mysqli_fetch_array($r);
	if (count($row)>0) {
		$tipo = $row['tipo'];
		$material = $row['material'];
		$idestado = $row['idestadores'];
		$estado = $row['estado'];
		$orden = $row['idorden'];
		$fecha = $row['fecha'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Ver Reserva</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=aprove">
				...
				<div id="tabs">
					...
					<div id="tabs-1">
						<table class="data-ro" id="res-srch-header">
							<tr>
								<td class="title">ID:</td><td class="id"><?php echo htmlspecialchars($id)?>&nbsp;&nbsp;-&nbsp;[Creado: <?php echo htmlspecialchars($created)?>&nbsp;|&nbsp;Modificado: <?php echo htmlspecialchars($modified)?>]&nbsp;-</span></td>
...
default:
	...
	/*if($_POST['delState']){
		...
		$sql = "SELECT s.id,s.numero,s.idorden,o.numero orden, eo.nombre estadoot, ex.nombre eecc,z.nombre zona, d.nombre depto,l.nombre localidad,
    s.fecha,s.tipo,DATE_FORMAT(s.create_date,'%Y-%m-%d') creado, s.cantidad,e.nombre estado,s.active,m.codigo,m.item,o.pm_orden,o.pm_reserva
    FROM reservasretal s, estadores e, ordenes o,estadoot eo, contratos c,eecc ex, zonas z, deptos d,localidades l,material m
    WHERE s.idorden=o.id $variable AND s.idestadores=e.id AND o.idestadoot=eo.id AND o.idcontrato=c.id AND c.ideecc=ex.id AND o.idzona=z.id AND o.iddepto=d.id AND o.idlocalidad=l.id AND s.idmaterial=m.id $locationfilter".getALLSQLFilters().getSQLSort("s.create_date","DESC");
		$q = db_query($sql);
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Gestion de Reservas</h2></div>
		...
		<div class="actionbar">
			...
			<div class="noresultsbar"><?php echo $regCount==0?"No hay registros para mostrar!":""?></div>
			<div class="pagingbar">
