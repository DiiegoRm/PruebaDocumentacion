...
switch($_REQUEST["mode"]){
 case 'view':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT s.numero,s.tipo,s.fecha,s.idestadores,e.nombre estado,s.idorden,s.create_date,s.modify_date, CONCAT(m.codigo,'|',m.item) material,s.fecha,s.cantidad FROM `reservas` s, estadores e, material m WHERE s.idestadores=e.id AND s.idmaterial=m.id AND s.`id` = '$id'");
...
default:
	...
	/*if($_POST['delState']){
		...
		$trayfilter = $appuser->getTrayFilter("s.id","idreserva","bandejasres");
...
?>
<?php include_once "parts/res/frm.contabilizar.mas.inc.php" ?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
