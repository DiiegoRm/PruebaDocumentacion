...
case 'make':
	...
	if($idpp > 0){
		...
		$sql = "INSERT INTO ordenes
		 (`numero`,`fecha_solicitud`,`idestadoot`,`idsegmento`,`idtipoot`,`fecha_requerida`,`idcontrato`,`idzona`,`iddepto`,`idlocalidad`,`idtipored`,`nombre`,`direccion`,`idviabilidad`,`idpresupuesto`,`ds`,`epro`,`trs`,`idclaseproyecto`,`idtipoproyecto`,`idpep`,`notas_ing`,`notas`,`ideecc`,`eeccxresponsable`,`resp_movistar`,`resp_eecc`,`idcluster`,`idsubcluster,`hh_pasados`,`idmes`,`iddistribuidor`,`armario`,`cable`,`idpop`,`parprim`,`parsec`,`parkm`,`kmfibra`,`mtsducto`,`idvelmaxba`,`distarm`,`iddistcaja`,`viviendas`,`torres`,`bocas`,`verticales`,latitud,longitud,atrib_dist,atrib_arm,`create_user`,`modify_user`)
		SELECT '0',CURRENT_DATE,$OT_ST_ENCREACION,`idsegmento`,`idtipoot`,DATE_ADD(CURRENT_DATE,INTERVAL DATEDIFF(`fecha_requerida`,`fecha_solicitud`) DAY),`idcontrato`,`idzona`,`iddepto`,`idlocalidad`,`idtipored`,`nombre`,`direccion`,$idvb,`id`,`ds`,`epro`,`trs`,`idclaseproyecto`,`idtipoproyecto`,null,`notas`,'Orden Iniciada',`ideecc`,`eeccxresponsable`,`resp_movistar`,`resp_eecc`,`idcluster`,`idsubcluster`,`hh_pasados`,`idmes`,`iddistribuidor`,`armario`,`cable`,`idpop`,`parprim`,`parsec`,`parkm`,`kmfibra`,`mtsducto`,`idvelmaxba`,`distarm`,`iddistcaja`,`viviendas`,`torres`,`bocas`,`verticales`,latitud,longitud,atrib_dist,atrib_arm,$appuser->uid,$appuser->uid
	...
	if(db_query($sql) > 0){
		...
		while ($row = mysqli_fetch_array($r)) {
			...
			if(copy(realpath(PP_FILE_PATH. DIRECTORY_SEPARATOR .basename($row['archivo'])),realpath(OT_FILE_PATH. DIRECTORY_SEPARATOR .basename($filename)))) {
...
break;
 case 'clone':
	...
	if($id > 0){
		$sql = "INSERT INTO ordenes
		 (`numero`,`fecha_solicitud`,`idestadoot`,`idsegmento`,`idtipoot`,`fecha_requerida`,`idcontrato`,`idzona`,`iddepto`,`idlocalidad`,`idtipored`,`nombre`,`direccion`,`idpresupuesto`,`ds`,`epro`,`trs`,`idclaseproyecto`,`idtipoproyecto`,`idpep`,`notas_ing`,`notas`,`ideecc`,`eeccxresponsable`,`resp_movistar`,`resp_eecc`,`idcluster`,`idsubcluster`,`hh_pasados`,`idmes`,`iddistribuidor`,`armario`,`cable`,`idpop`,`parprim`,`parsec`,`parkm`,`kmfibra`,`mtsducto`,`idvelmaxba`,`distarm`,`iddistcaja`,`viviendas`,`torres`,`bocas`,`verticales`,latitud,longitud,atrib_dist,atrib_arm,`create_user`,`modify_user`)
...
break;
 case 'edit':
	...
	$r =  db_query("SELECT o.*,p.mo pep_mo,p.cable pep_cable,p.otros pep_otros FROM `ordenes` o LEFT JOIN peps p ON o.idpep=p.id WHERE o.`id` = '$id' $userfilter");

	if ($row = mysqli_fetch_array($r)) {
		$estadoot = "En Modificacion";
		$numero = $row['numero'];
		...
		$txtidcluster = $row['idcluster'];
		$txtidsubcluster = $row['idsubcluster'];
		$txthh_pasados = $row['hh_pasados'];
		$txtidmes = $row['idmes'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Ver Orden de Trabajo 'Modificacion'</h2></div>
...
?>
 ...
 default:
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					...
					db_query("DELETE FROM `tareas` WHERE id IN (SELECT * FROM (
						SELECT p.id FROM tareas p, cronograma c WHERE p.idcrono=c.id AND c.idorden={$del[$i]}) del)");
		...
		$userfilter = (!$appuser->isAdmin())?"AND o.create_user=$appuser->uid ":"";
		$sql = "SELECT o.id,o.numero,o.fecha_solicitud,o.fecha_requerida,o.nombre,o.active,eo.nombre estado,
		tot.nombre req,ee.nombre eecc,z.nombre zona, d.nombre depto,l.nombre localidad,tr.nombre red,cp.nombre proyecto,
		tp.tmo,tp.tma,IF(o.fecha_requerida BETWEEN DATE_SUB(current_timestamp,INTERVAL 2 DAY)
		AND CURRENT_TIMESTAMP,'amarillo',IF(CURRENT_TIMESTAMP > o.fecha_requerida,'rojo','verde')) alerta
		...
		WHERE o.idtipoot=tot.id $userfilter AND o.idestadoot = $OT_ST_ENCREACION
		AND o.idestadoot=eo.id".getAllSQLFilters().getSQLSort("o.fecha_solicitud","DESC");
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("o.numero"=>"Numero","o.fecha_solicitud"=>"Solicitada","o.fecha_requerida"=>"Requerida","estado"=>"Estado",
			"ee.nombre"=>"EECC","z.nombre"=>"Zona","d.nombre"=>"Depto","l.nombre"=>"Localidad","tot.nombre"=>"Tipo","o.nombre"=>"Nombre",
			"tr.nombre"=>"TipoRed","cp.nombre"=>"Proyecto","tp.tmo"=>"Total MO","tp.tma"=>" Total MA");
