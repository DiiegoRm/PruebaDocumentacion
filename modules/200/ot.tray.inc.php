...
switch(clean_input($_REQUEST["mode"])){
	...
	case 'edit':
		$id = decrypt(getVal(clean_input($_GET['id'])));
  		$id = mysqli_real_escape_string($dbsgp,$id);//KIUWAN
  
		var_dump($OT_ST_ENCREACION);
				...
				<div class="info">
					<div class="formpage">
						<div class="outerbox">
							...
							<div class="messagebar"><span id="message" class="error"></span></div>
									...
									if(!$appuser->isInState($idestadoot,$OT_ST_ENAPROBACIONECONOMICA)){?>
										...
										$(function() {
											...
											$( "#tabs" ).tabs({
												...
												<?php if(strlen(clean_input($_GET['tab']))>0)echo htmlspecialchars(",active:".clean_input($_GET['tab'])); ?>
											});
										});
										</script>
										<div id="tabs">
											<ul>
												...
												<?php } ?>
												<?php if($appuser->isInState($idestadoot,$OT_ST_ENREPROGRAMACION) || $appuser->isInState($idestadoot,$OT_ST_APLAZADA) || $appuser->isInState($idestadoot,$OT_ST_PENDIENTEMATERIALES) || $appuser->isInState($idestadoot,$OT_ST_ENAPROBACIONECONOMICA)){ ?>
												...
												<!-- DF - Adicion -->
												<li><a href="#tabs-12">Adici&oacute;n</a></li>
											...
											<div id="tabs-8" style="display: none;"><?php include_once "parts/ot/tab.pedidos.rw.inc.php"; ?></div>
											<!-- DF - Adicion -->
											<div id="tabs-12" style="display: none;"><?php include_once "parts/ot/tab.adicion.rw.inc.php"; ?></div>				
											<?php  if($appuser->isInState($idestadoot,$OT_ST_APLAZADA)){ ?>
												<div id="tabs-1" style="display: none;">
													<?php include_once "parts/ot/tab.orden.rw.inc.php"; ?>
												</div>
											<?php }?>
											<div id="tabs-11" style="display: none;"><?php include_once "parts/ot/tab.equipos.inc.php"; ?></div>
											<div id="tabs-2" style="display: none;"><?php include_once "parts/ot/tab.totales.xw.inc.php"; ?></div>
											<?php if($liqTotal==0 || ($liqTotal>0 && ($estadoliq==$LIQ_ST_RECHAZADA || $estadoliq==$LIQ_ST_CANCELADA))){ ?>
												<div id="tabs-3" style="display: none;"><?php include_once "parts/ot/tab.baremos.xw.inc.php"; ?></div>
											<?php }?>
											<div id="tabs-4" style="display: none;"><?php include_once "parts/ot/tab.materiales.xw.inc.php"; ?></div>
											<?php if(!($liqTotal>0 && $estadoliq!=$LIQ_ST_RECHAZADA && $estadoliq!=$LIQ_ST_CANCELADA)){ ?>
												<div id="tabs-5" style="display: none;"><?php include_once "parts/ot/tab.solicitudes.rw.inc.php"; ?></div>
											<?php }?>
											<?php if($appuser->isInState($idestadoot,$OT_ST_ENREPROGRAMACION) || $appuser->isInState($idestadoot,$OT_ST_APLAZADA) || $appuser->isInState($idestadoot,$OT_ST_PENDIENTEMATERIALES) || $appuser->isInState($idestadoot,$OT_ST_ENAPROBACIONECONOMICA)){ ?>
												<div id="tabs-6" style="display: none;"><?php include_once "parts/ot/tab.cronograma.xw.inc.php"; ?><div id="ganttChart"></div><br /><br /><div id="eventMessage"></div></div>
											<?php }?>
											<div id="tabs-7" style="display: none;"><?php include_once "parts/ot/tab.causacion.rw.inc.php"; ?></div>
											<div id="tabs-9" style="display: none;"><?php include_once "parts/ot/tab.adjuntos.rw.inc.php"; ?></div>
											<?php if(!$appuser->isInState($idtipoot,"$OT_TIPO_DESIGN,$OT_TIPO_DIAGNOSTICO") && (!$appuser->isInState($idestadoot,"$OT_INACTIVA"))){?>
												<div id="tabs-10" style="display: none;"><?php include_once "parts/ot/tab.registro.rw.inc.php"; ?></div>
											<?php } ?>
										</div>
									<?php } else { ?>
										<script type="text/javascript">
											$(function() {
												...
												$( "#tabs" ).tabs({
													...
													<?php if(strlen(clean_input($_GET['tab']))>0)echo ",active:".htmlspecialchars(clean_input($_GET[tab])).""; ?>
												});
											});
										</script>
										<div id="tabs">
...
$txtidmes = getPostStr('txtidmes');
	...
	/* Campos Nuevos */
	$txtregion = getPostStr('txtregion');
	$txtpoligono = getPostStr('txtpoligono');
	$txtcomuna = getPostStr('txtcomuna');
	$txtcluster = getPostStr('txtcluster');
	$txtCableOrd = getPostStr('txtCableOrd');
	$txtcentral = getPostStr('txtcentral');
	$txtConversor = getPostStr('txtConversor');
	$txtsubcluster = getPostStr('txtsubcluster');
	$txttipozona = getPostStr('txttipozona');
	/* End */


	if(hasVal($txtTipoOT)&&hasVal($txtSegmento)){
		$uid = $appuser->uid;
		$sql_ins = db_query("UPDATE `ordenes` SET `idsegmento`=$txtSegmento,`idtipoot`=$txtTipoOT,
							`hh_pasados`=$txthh_pasados,`idcluster`=$txtidcluster,`idsubcluster`=$txtsubcluster,
							`idmes`=$txtidmes,`idcontrato`=$txtContrato,`eeccxresponsable`=$txtResponsable,
							`idzona`=$txtZona,`iddepto`=$txtDepto,`idlocalidad`=$txtLocalidad,
							`idtipored`=$txtTipoRed,`nombre`=$txtNombre,`direccion`=$txtDireccion,
							`ds`=$txtDs,`epro`=$txtEpro,`trs`=$txtTrs,`idclaseproyecto`=$txtClase,
							`idtipoproyecto`=$txtTipo,idpep=$txtPEP,resp_movistar=$txtRespMovistar,
							resp_eecc=$txtRespEECC,iddistribuidor=$txtDistribuidor,idpop=$txtPOP,
							armario=$txtArmario,cable=$txtCable,parprim=$txtPares1,parsec=$txtPares2,
							idvelmaxba=$txtVelMax,distarm=$txtDist1,iddistcaja=$txtDist2,
							viviendas=$txtVivienda,torres=$txtTorres,bocas=$txtBocas,
							verticales=$txtVerticales,notas_ing=$txtObs,latitud=$txtLatitud,
							longitud=$txtLongitud,atrib_dist=$CheckAtriDist,
							atrib_arm=$CheckAtriArm,
							
							sub_cluster=$txtsubcluster,idcentral=$txtcentral,
							idregion=$txtregion,idpoligono=$txtpoligono,id_cluster=$txtcluster,
							idcomuna=$txtcomuna,idtipozona=$txttipozona,idcable=$txtCableOrd,
...
echo "<hr>";
var_dump($STC);
var_dump($appuser->isInGroup($STC));
var_dump($appuser->isAdmin());
var_dump($appuser->isBtwRole($GESTIONAR_PEDIDOS,$FACTURA));
if($appuser->isInGroup($STC) || $appuser->isAdmin()||$appuser->isBtwRole($GESTIONAR_PEDIDOS,$FACTURA)){
