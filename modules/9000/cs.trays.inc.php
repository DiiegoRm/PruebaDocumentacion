...
ob_start();
	...
	$sql = "SELECT l.id,o.numero orden, l.numero, e.nombre estado,l.active FROM liquidaciones l, estadoliq e, ordenes o WHERE l.idestadoliq=e.id AND l.idorden=o.id".getAllSQLFilters().getSQLSort("l.fecha_liquidacion","DESC");
	$q = db_query($sql);
	$regCount = mysqli_num_rows($q);

	$maxPage = ceil($regCount/$rowsxPage);
	$rowFrom = (($pageNO-1) * $rowsxPage);
	$fields = array("o.numero"=>"Orden","l.numero"=>"Numero","e.nombre"=>"Estado",""=>"Bandejas","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Causaciones::Bandejas</h2></div>
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					$bandejas = getSQLNames("SELECT g.nombre FROM grupos g, bandejasliq b WHERE b.idliquidacion=$row[id] AND b.idgrupo=g.id");
					...
					if($row['active']=='Si'){
						echo "<td><a href=\"?menu=$MENU_SRCH_CS&amp;mode=view&amp;id=".encrypt(htmlspecialchars($row['id']))."\">".htmlspecialchars($row[numero])."</a></td>\n";
					} else {
						echo "<td>".htmlspecialchars($row[numero])."</td>\n";
					}
					echo "<td>".htmlspecialchars($row[estado])."</td>\n";
					echo "<td>".htmlspecialchars($bandejas)."</td>\n";
