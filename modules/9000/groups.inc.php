...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($nombre)){
		$sql_update = db_query("INSERT INTO `grupos` (`nombre`) VALUES ($nombre)");
 ...
 case 'save':
	...
	$nombre=getStrVal($_POST['txtNombre']);
	if(hasVal($nombre)){
		$sql_update = db_query("UPDATE `grupos` SET `nombre`=$nombre,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Grupo</h2></div>
 ...
 <script type="text/javascript" src="js/val/grupos.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Grupo</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					if($del[$i]>16)$sql_update = db_query("DELETE FROM `grupos` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
		...
		$sql = "SELECT * FROM `grupos`".getSQLFilters('WHERE').getSQLSort();
		$q = db_query($sql);
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Grupos</h2></div>
