...
ob_start();
	...
	$sql =" SELECT o.id,o.numero,o.nombre,o.active,e.nombre estado, t.nombre tipo/*,
	GROUP_CONCAT(g.nombre SEPARATOR ',') bandejas*/
	FROM ordenes o
	/*LEFT JOIN liquidaciones liq on o.id=liq.idorden
	LEFT JOIN bandejasot b ON b.idorden=o.id
	CROSS JOIN bandejasliq bliq on liq.id=bliq.idliquidacion
	INNER JOIN grupos g on b.idgrupo=g.id OR bliq.idgrupo=g.id*/
	, estadoot e, tipoot t, contratos c
	WHERE o.idestadoot > $OT_ST_ENCREACION $eeccfilter $locationfilter AND o.idestadoot=e.id AND o.idtipoot=t.id
	AND o.idcontrato=c.id".getAllSQLFilters()." GROUP BY o.id".getSQLSort("o.fecha_solicitud","DESC");
	$q = db_query($sql);
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					...
					if($row['active']=='Si'){
						echo "<td><a href=\"?menu=206&amp;mode=show&amp;id=".encrypt(htmlspecialchars($row['id']))."\">".htmlspecialchars($row[numero])."</a></td>\n";
						//echo "<td><a href=\"?menu=206 id=".encrypt($row['id'])."\">$row[numero]</a></td>\n";
					} else {
