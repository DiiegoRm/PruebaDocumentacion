...
switch($_REQUEST["mode"]){
 case 'view':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT p.numero,p.idorden,p.idestadoped,p.create_date,p.fecha_programada,p.fecha_entrega,p.traslado,CONCAT(m.codigo,'|',m.item) material
  FROM `pedidosxorden` p, material m
  WHERE p.idmaterial=m.id AND p.`id` = '$id'");
	$row = mysqli_fetch_array($r);
	if (count($row)>0) {
		$numero = $row['numero'];
		$idestado = $row['idestadoped'];
		$estado = getNameById("estadoped",$idestado);
		$orden = $row['idorden'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Ver Pedido Material</h2></div>
...
default:
	...
	$sql = "SELECT p.id,p.numero,p.idorden,o.numero orden,e.nombre estado,DATE_FORMAT(p.create_date,'%Y-%m-%d') create_date,p.fecha_programada,ex.nombre eecc,
  z.nombre zona, d.nombre depto, l.nombre localidad,m.codigo,m.item,m.unidad,p.traslado,cantidad,p.fecha_entrega,p.active,IF(e.nombre!='Entregado' AND e.nombre!='Cancelado',IF(CURRENT_DATE > p.fecha_programada,'rojo',IF(DATEDIFF(p.fecha_programada,CURRENT_DATE) <= 2,'amarillo','verde')),'') alerta,o.pm_orden
  FROM pedidosxorden p, ordenes o, estadoped e,material m,contratos c, eecc ex,zonas z,deptos d,localidades l
  WHERE p.idorden=o.id AND p.idestadoped=e.id AND p.idmaterial=m.id AND o.idcontrato=c.id AND c.ideecc=ex.id AND o.idzona=z.id AND o.iddepto=d.id AND o.idlocalidad=l.id $locationfilter".getAllSQLFilters().getSQLSort("p.create_date","DESC");
	$q = db_query($sql);
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Pedidos de Materiales</h2></div>
