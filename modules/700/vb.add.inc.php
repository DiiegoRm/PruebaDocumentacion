...
switch($_REQUEST["mode"]){
 case 'new':
	...
	//desarrolo migracion numero CTO`S
	...
	$txthogarespas = getVal($_POST['txthogarespas'],"null");
	...
	if(hasVal($txtTipoVB)&&hasVal($txtEntrega)){
		$sql_update = db_query("INSERT INTO `viabilidades` (numero,fecha_solicitud,fecha_requerida,idestadovb,entrega,idtipovb,idjefatura,idjefe,
      idregion,iddepto,idlocalidad,idsegmento,nombre,direccion,constructora,contacto,telefono,ds,idproyectovb,lb,ba,tv,estrato,total_viviendas,etapa,viviendas_etapa,notas_seg,pago,periodo ,create_user,modify_user,notas,numcto,cable,idcentral,conversor,idpoligono,idcomuna,idcluster,hogares_pasados,id_region,subcluster,idtipo_vb,idtipozona)
							   VALUES ('0',$txtDate,$fecha_requerida,$txtEstado,'$txtEntrega',$txtTipoVB,$txtJefatura,$txtJefe,$txtRegion,$txtDepto,$txtLocalidad,
                   $txtSegmento,$txtNombre,$txtDireccion,$txtConstructora,$txtContacto,$txtTelefono,$txtDs,$txtProyecto,$txtLB,$txtBA,$txtTV,$txtEstrato,$txtViviendas,$txtEtapa,$txtViviendasEtapa,$txtObs,'0',DATE_FORMAT(CURRENT_TIMESTAMP,'%Y%m') ,$uid,$uid,'Viabilidad Iniciada',$txtnumerocto,$txtCable,$txtCentral,$txtconversor,$txtPoligono,$txtComuna,$txtcluster,$txthogarespas,$txtregiion,$txtsubclus,$txttipo_vb,$txtTipoZona)");
		$lastvb = getLastId();
        ...
        $uptname=$_POST["uploader_${i}_tmpname"];
				if (copy(realpath(UPLOAD_TMP_DIR. DIRECTORY_SEPARATOR .basename($uptname)),basename(VB_FILE_PATH. DIRECTORY_SEPARATOR .basename("$lastvb.".$uptname)))) {
 ...
 case 'save':
    ...
    $fecha_requerida=getStrVal($_POST['txtRequerida'],"null");
	...
	$txtDs = getPostStr('txtDs');
	$txtTelefono = getPostStr('txtTelefono');
	$txtnumerocto = getVal($_POST['txtnumerocto'],"null");//-----numero CTO�S
	$txtLB = getVal($_POST['txtLB'],"0");
	$txtBA = getVal($_POST['txtBA'],"0");
	$txtTV = getVal($_POST['txtTV'],"0");
	$txtEstrato = getStrVal($_POST['txtEstrato'],"null");
	...
	$completed = hasVal($txtSegmento)&&hasVal($txtJefatura)&&hasVal($txtJefe)&&hasVal($txtRegion)&&
			...
			hasVal($txtNombre)&&hasVal($txtDireccion);
	...
	if(hasVal($txtTipoVB)&&hasVal($txtEntrega)){
		...
		$sql = "UPDATE `viabilidades` SET fecha_solicitud=$txtDate,fecha_requerida=$fecha_requerida,entrega='$txtEntrega',
			...
			constructora=$txtConstructora,contacto=$txtContacto,telefono=$txtTelefono,ds=$txtDs,
			...
			notas_seg=$txtObs,modify_user=$uid,`modify_date`=CURRENT_TIMESTAMP,cable=$txtCable,idcentral=$txtCentral,
			conversor=$txtconversor,idpoligono=$txtPoligono,idcomuna=$txtComuna,idcluster=$txtcluster,
			hogares_pasados=$txthogarespas,id_region=$txtregiion,subcluster=$txtsubclus,idtipo_vb=$txttipo_vb,idtipozona=$txtTipoZona";
	...
	if($appuser->hasSegmentoVB()){
      ...
      $row = mysqli_fetch_array($r);
			if (count($row)>0) {
				...
				$Ds = $row['ds'];
				$numcto = $row['numcto'];//-----numero CTO�S
				$lb = $row['lb'];
				$ba = $row['ba'];
				$tv = $row['tv'];
				$estrato = $row['estrato'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Viabilidad</h2></div>
 ...
 case 'edit':
	...
	if (count($row)>0) {
		...
		$completed = hasVal($idsegmento)&&hasVal($idjefatura)&&hasVal($idjefe)&&hasVal($idregion)&&
			...
			hasVal($nombre)&&hasVal($direccion);
...
default:
	...
	if($_POST['delState']){
		...
		$sql = "SELECT v.id,v.numero,v.fecha_solicitud,v.nombre,v.active,ev.nombre estado,u.nombre solicitante, s.nombre segmento, v.cable cable,c.nombre central
		,v.conversor,r.nombre region,p.nombre poligono,co.nombre Municipio,clu.nombre cluster_ftth,v.hogares_pasados hogares_pasados,v.subcluster,t.nombre tipovb,z.nombre tipozona    
		FROM viabilidades v 
        LEFT join segmentos s on v.idsegmento=s.id
        LEFT join usuarios u on v.create_user= u.id
        LEFT join estadovb ev on v.idestadovb=ev.id
        LEFT JOIN central c on v.idcentral=c.id 
        LEFT JOIN poligono p on v.idpoligono=p.id
        LEFT JOIN comuna co on v.idcomuna=co.id
        LEFT JOIN cluster clu on v.idcluster=clu.id 
        LEFT JOIN region r on v.id_region=r.id 
        LEFT JOIN tipo_vb t on v.idtipo_vb=t.id 
        LEFT JOIN tipozona z on v.idtipozona=z.id 
		WHERE v.idestadovb = $VB_ST_CREACION AND v.idestadovb=ev.id AND v.fecha_solicitud>='2017-03-01' AND v.create_user=u.id AND v.idsegmento=s.id AND v.active='Si'
		$userfilter".getAllSQLFilters().getSQLSort();
		...
		$fields = array("v.numero"=>"Numero","v.fecha_solicitud"=>"Solicitada","v.nombre"=>"Nombre","ev.nombre"=>"Estado","u.nombre"=>"Solicitante","s.nombre"=>"Segmento","v.active"=>"Activo");
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Viabilidades "En Creaci&oacute;n"</h2></div>
