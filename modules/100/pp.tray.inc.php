...
case 'clone':
		$id=getVal($_GET['id'],"0");
	if($id > 0){
		$sql = "INSERT INTO presupuesto (numero,fecha_solicitud,estado,idsegmento,idtipoot,fecha_requerida,idcontrato,idzona,iddepto,idlocalidad,idtipored,
			nombre,direccion,ds,epro,trs,idclaseproyecto,idtipoproyecto,notas,ideecc,eeccxresponsable,resp_movistar,resp_eecc,iddistribuidor,armario,
			cable,idpop,parprim,parsec,parkm,kmfibra,mtsducto,idvelmaxba,distarm,iddistcaja,viviendas,torres,bocas,verticales,latitud,longitud,atrib_dist,atrib_arm,create_user,modify_user)
		SELECT '0',CURRENT_DATE,'INICIADO',idsegmento,idtipoot,fecha_requerida,idcontrato,idzona,iddepto,idlocalidad,idtipored,nombre,direccion,ds,epro,trs,idclaseproyecto,idtipoproyecto,'Presupuesto Iniciado',ideecc,eeccxresponsable,resp_movistar,resp_eecc,iddistribuidor,armario,cable,idpop,parprim,parsec,parkm,kmfibra,mtsducto,idvelmaxba,distarm,
		iddistcaja,viviendas,torres,bocas,verticales,latitud,longitud,atrib_dist,atrib_arm,$appuser->uid,$appuser->uid
	  FROM presupuesto  WHERE estado='CREADO' AND id = $id";
		...
		if(count($sqlb)> 0){
			...
			db_query("INSERT INTO totalesxpresupuesto(idpresupuesto,fdepto,cdirecto,claseh,costoaiu,utilidadp,utilidad,ivap,iva,tpb,tmo,tca,tma,totros,tpry)
			SELECT $lastot,fdepto,cdirecto,claseh,costoaiu,utilidadp,utilidad,ivap,iva,tpb,tmo,tca,tma,totros,tpry FROM totalesxpresupuesto WHERE idpresupuesto=$id");
...
if($txtMake == "100"){
	...
	if(hasVal($txtTipoOT)&&hasVal($txtSegmento)){
		$uid = $appuser->uid;
		$sql_ins = db_query("UPDATE presupuesto
			SET fecha_solicitud=CURRENT_DATE,idsegmento=$txtSegmento,idtipoot=$txtTipoOT,idcontrato=$txtContrato,idzona=$txtZona,
			iddepto=$txtDepto,idlocalidad=$txtLocalidad,idtipored=$txtTipoRed,nombre=$txtNombre,direccion=$txtDireccion,ds=$txtDs,
			epro=$txtEpro,trs=$txtTrs,idclaseproyecto=$txtClase,idtipoproyecto=$txtTipo,resp_movistar=$txtRespMovistar,
			resp_eecc=$txtRespEECC,iddistribuidor=$txtDistribuidor,idpop=$txtPOP,armario=$txtArmario,cable=$txtCable,parprim=$txtPares1,
			parsec=$txtPares2,idvelmaxba=$txtVelMax,distarm=$txtDist1,iddistcaja=$txtDist2,viviendas=$txtVivienda,torres=$txtTorres,bocas=$txtBocas,
			verticales=$txtVerticales,notas=$txtObs,latitud=$txtLatitud,longitud=$txtLongitud,atrib_dist=$CheckAtriDist,atrib_arm=$CheckAtriArm,
			port=$txtport WHERE id=$txtId");
		if($sql_ins > 0){
...
break;
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Ver Presupuesto 'En Creacion'</h2></div>
...
echo "hi";
	...
	if($_POST['delState']){
		...
		$sql = "SELECT o.id,o.numero,o.fecha_solicitud,o.fecha_requerida,o.nombre,o.active,o.estado,tot.nombre req,ee.nombre eecc,z.nombre zona, d.nombre depto,l.nombre localidad,tr.nombre red,cp.nombre proyecto,tp.tmo,tp.tma
		FROM presupuesto o LEFT JOIN eecc ee ON o.ideecc=ee.id LEFT JOIN zonas z ON o.idzona=z.id LEFT JOIN deptos d ON o.iddepto=d.id LEFT JOIN localidades l ON o.idlocalidad=l.id LEFT JOIN tipored tr ON o.idtipored=tr.id LEFT JOIN claseproyecto cp ON o.idclaseproyecto=cp.id LEFT JOIN totalesxpresupuesto tp ON tp.idpresupuesto=o.id,tipoot tot
		WHERE o.estado='INICIADO' AND o.fecha_solicitud>='2017-03-01' AND o.active='Si' $userfilter AND o.idtipoot=tot.id".getAllSQLFilters().getSQLSort("o.create_date","DESC");
		$q = db_query($sql);
