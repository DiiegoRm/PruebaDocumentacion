...
switch($_REQUEST["mode"]){
 case 'save':
	...
	$txtEECC=getVal($_POST['txtEECC']);//
  $txtEECC=mysqli_real_escape_string($dbsgp,$txtEECC);//KIUWAN
	if($txtEECC != ''){
		$sql = "UPDATE `viabilidades` SET ideecc=$txtEECC WHERE id=$id";
		$sql_update = db_query($sql);
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		$sql = "UPDATE `viabilidades` SET ideecc=null WHERE id=$id";
		$sql_update = db_query($sql);
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
 break;
 case 'update':
  ...
  $id=mysqli_real_escape_string($dbsgp,$id);//KIUWAN
	$r =  db_query("SELECT numero,iddepto,ideecc FROM viabilidades WHERE id=$id");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Cambiar EECC a [<?php echo htmlspecialchars($numero)?>] </h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width: 50%">
					...
					<tr>
		<td class="title"><span class="required">*</span>Nuevo EECC:</td>
		<td class="input"><?php echo getComboBox("SELECT e.id,e.nombre,e.active FROM eecc e, contratos c, zonaxdepto z WHERE c.idzona=z.idzona AND c.ideecc=e.id AND z.iddepto=".htmlspecialchars($iddepto)."","txtEECC",htmlspecialchars($ideecc));?></td>
					</tr>
				</table>
				<br class="clear"/>
...
default:
  ...
  $pageNO=mysqli_real_escape_string($dbsgp,$pageNO);//KIUWAN
	$rowsxPage=100;
		...
		$sql = "SELECT v.id,v.numero,v.fecha_solicitud,v.entrega,v.fecha_requerida,v.nombre,v.fecha_respuesta,v.fecha_presupuesto,
		v.active,ev.nombre estado,u.nombre solicitante, s.nombre segmento,tv.nombre requerimiento,d.nombre depto,l.nombre localidad, r.nombre region, 
		IF(v.idestadovb=$VB_ST_ESTUDIO,IF(v.fecha_requerida BETWEEN DATE_SUB(current_timestamp,INTERVAL 1 DAY) AND current_timestamp,'amarillo',
		IF(current_timestamp > v.fecha_requerida,'rojo','verde')),'') alerta, 
		IFNULL(e.nombre,'-') eecc,p.numero presupuesto,v.idpresupuesto,o.numero orden,v.idorden FROM viabilidades v 
		LEFT JOIN ordenes o ON (v.idorden=o.id) LEFT JOIN presupuesto p ON (v.idpresupuesto=p.id) 
		LEFT JOIN eecc e ON (v.ideecc=e.id), estadovb ev,usuarios u, segmentos s,tipovb tv, localidades l, deptos d, regiones r 
		WHERE $statefilter $locationfilter AND v.idestadovb=ev.id AND v.create_user=u.id 
		AND v.idtipovb=tv.id AND v.idlocalidad=l.id AND v.iddepto=d.id AND v.idsegmento=s.id AND v.idregion=r.id".getAllSQLFilters().getSQLSort("v.create_date","DESC");
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Cambio de EECC Viabilidades</h2></div>
