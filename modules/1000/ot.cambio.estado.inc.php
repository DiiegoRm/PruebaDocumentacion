...
switch($_REQUEST["mode"]){
 case 'save':
  ...
  $txtEstado=mysqli_real_escape_string($dbsgp,$txtEstado);//KIUWAN
	if(hasVal($txtEstado)){
		$sql = "UPDATE `ordenes` SET idestadoot=$txtEstado,notas='Cambio de Estado Manual',modify_user=$appuser->uid,modify_date=CURRENT_TIMESTAMP WHERE id=$id";
		$sql_update = db_query($sql);
		$sql_update = db_query("DELETE FROM equipos_ot_terminar WHERE id_ot = $id");
 ...
 case 'update':
  ...
  $id=mysqli_real_escape_string($dbsgp,$id);//KIUWAN
	$r =  db_query("SELECT o.numero,o.idestadoot,e.nombre estado FROM ordenes o, estadoot e where o.idestadoot=e.id AND o.id=$id");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		...
		<td class="input"><?php echo getComboBox("SELECT id,nombre,active FROM estadoot","txtEstado",htmlspecialchars($idestadoot));?></td>
					</tr>
				</table>
				<br class="clear"/>
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Cambio de Estado Ordenes</h2></div>
