										...
										<?php
											...
											if (mysqli_num_rows($dataq) != 0) {
												...
												while($rowq = mysqli_fetch_array($dataq)){?>
													...
													<td><a href="<?php echo SOL_FILE_WEB . "/".htmlspecialchars($rowq[archivo]).""; ?>"><?php echo htmlspecialchars($rowq['titulo']); ?></a></td>
													</tr>
												<?php
												}
											}
...
default:
	...
	$locationfilter = $appuser->getLocationFilterOT("o.");
		$sql = "SELECT s.id,s.idorden,o.numero orden,c.numero contrato,ec.nombre eecc,z.nombre zona, d.nombre depto,l.nombre localidad,s.valor,s.create_date creado, u.nombre creador,e.nombre estado,s.active,UNIX_TIMESTAMP(CURRENT_TIMESTAMP)-IFNULL(UNIX_TIMESTAMP(s.modify_date),UNIX_TIMESTAMP(s.create_date)) secs FROM solicitudesh s, estadosol e, ordenes o, usuarios u, contratos c,deptos d,eecc ec,zonas z,localidades l WHERE s.idorden=o.id AND s.idestadosol=e.id $variable AND o.idcontrato=c.id AND o.iddepto=d.id AND s.create_user=u.id AND o.ideecc=ec.id AND o.idzona=z.id AND s.active='Si' AND o.idlocalidad=l.id $locationfilter".getAllSQLFilters().getSQLSort("s.create_date","DESC");
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("o.numero"=>"Orden","s.id"=>"Solicitud","c.numero"=>"Contrato","ec.nombre"=>"EECC","z.nombre"=>"Zona","d.nombre"=>"Depto","l.nombre"=>"Localidad","s.valor"=>"Valor","s.create_date"=>"Fecha Ingreso","u.nombre"=>"Solicitante","e.nombre"=>"Estado Solicitud","s.modify_date"=>"Tiempo","s.active"=>"Activo");
...
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Solicitudes Puntos Pactados</h2></div>
