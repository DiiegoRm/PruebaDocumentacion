...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name) && hasVal($txtMarca)){
		$sql_update = db_query("INSERT INTO `tipoequipo` (`nombre`,`marca_id`) VALUES ($name,$txtMarca)");
 ...
 case 'save':
	...
	if(hasVal($name) && hasVal($txtMarca)){
		$sql_update = db_query("UPDATE `tipoequipo` SET `nombre`=$name,`marca_id` = $txtMarca,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Funcionalidad</h2></div>
 ...
 <script type="text/javascript" src="js/val/tipoequipo.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `tipoequipo` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Funcionalidad</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Marca:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM marca ORDER BY nombre ASC",'txtMarca',htmlspecialchars($row['marca_id']));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `tipoequipo` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `tipoequipo` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `tipoequipo` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT te.*, m.nombre as marca FROM tipoequipo te INNER JOIN marca m ON m.id = te.marca_id".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("id"=>"ID","te.nombre"=>"Funcionalidad","m.nombre"=>"Marca","te.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Funcionalidades</h2></div>
