...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name) && hasVal($idregion)){
		$sql_update = db_query("INSERT INTO `cluster` (`nombre`, `idcomuna`) VALUES ($name, $idregion)");
 ...
 case 'save':
	...
	if(hasVal($name)){
		$sql_update = db_query("UPDATE `cluster` SET `nombre` = $name, idcomuna=$idregion, `modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar cluster FTTH</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Municipio:</span></td>
						<td class="input"><?php echo getComboAdjust("SELECT id,nombre,active FROM comuna",'txtRegion');?></td>
					</tr>
				</table>
				<br class="clear"/>
 ...
 <script type="text/javascript" src="js/val/gestion_Poligono.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `cluster` WHERE `id` = '$id'");
	$row = mysqli_fetch_array($r);
	if (count($row)>0) {
		$name = $row['nombre'];
		$idregion = $row['idcomuna'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar cluster FTTH</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Municipio:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM comuna",'txtRegion',htmlspecialchars($idregion));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `cluster` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `cluster` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `cluster` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT a.id, a.nombre, b.nombre comuna, a.active, a.create_date 
				FROM cluster a 
				JOIN comuna b
				ON a.idcomuna = b.id
		...
		$fields = array("id"=>"ID","a.nombre"=>"Nombre cluster ffth","b.nombre"=>"Comuna","create_date"=>"Fecha creacion","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Cluster FTTH</h2></div>
