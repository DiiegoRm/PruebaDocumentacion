...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name) && hasVal($idregion)){
		$sql_update = db_query("INSERT INTO `poligono` (`nombre`, `idregion`) VALUES ($name, $idregion)");
 ...
 case 'save':
	...
	if(hasVal($name)){
		$sql_update = db_query("UPDATE `poligono` SET `nombre` = $name, idregion=$idregion, `modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar poligono</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						...
						<td class="input"><?php echo getComboAdjust("SELECT id,nombre,active FROM region",'txtRegion');?></td>
					</tr>
				</table>
				<br class="clear"/>
...
<?php
 ...
 case 'edit':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `poligono` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar poligono</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Region:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM region",'txtRegion',htmlspecialchars($idregion));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `poligono` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `poligono` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `poligono` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT a.id, a.nombre, b.nombre region, a.active, a.create_date 
				FROM poligono a 
				JOIN region b
				ON a.idregion = b.id
		...
		$fields = array("id"=>"ID","a.nombre"=>"Nombre poligono","b.nombre"=>"Region","create_date"=>"Fecha creacion","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Poligono</h2></div>
