...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($id)&&hasVal($name)&&hasVal($iddepto)){
		$sql_update = db_query("INSERT INTO `localidades` (id,`nombre`,`iddepto`) VALUES ($id,$name,$iddepto)");
 ...
 case 'save':
	...
	if(hasVal($name)&&hasVal($iddepto)){
		$sql_update = db_query("UPDATE `localidades` SET `nombre`=$name,iddepto=$iddepto,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Localidad</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					<tr>
						...
						<td class="input"><?php echo getInputField('txtId');?></td>
					</tr>
					<tr>
 ...
 <script type="text/javascript" src="js/val/localidad.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `localidades` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Localidad</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						...
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM deptos",'txtDepto',htmlspecialchars($iddepto));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `localidades` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `localidades` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `localidades` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT l.id,l.nombre,l.active,d.nombre as depto FROM localidades l, deptos d WHERE l.iddepto=d.id".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("l.id"=>"ID","l.nombre"=>"Localidad","d.nombre"=>"Departamento","l.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Localidades</h2></div>
