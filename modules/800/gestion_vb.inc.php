...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name)){
		$sql_update = db_query("INSERT INTO `tipo_vb` (`nombre`) VALUES ($name)");
 ...
 case 'save':
	...
	if(hasVal($name)){
		$sql_update = db_query("UPDATE `tipo_vb` SET `nombre` = $name, `modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <script type="text/javascript" src="js/val/gestion_tipo_vb.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `tipo_vb` WHERE `id` = '$id'");
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `tipo_vb` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `tipo_vb` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `tipo_vb` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT id, nombre, active, create_date FROM tipo_vb WHERE 1=1 ".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("id"=>"ID","nombre"=>"Nombre tipo vb","create_date"=>"Fecha creacion","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Tipo vb</h2></div>
