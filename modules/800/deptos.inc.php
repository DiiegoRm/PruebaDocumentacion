...
switch($_REQUEST["mode"]){
 case 'new':
	...
	$txtFactor=getVal($_POST['txtFactor']);
	if(hasVal($name)&&hasVal($txtFactor)){
		$sql_update = db_query("INSERT INTO `deptos` (`nombre`,`factor`) VALUES ($name,$txtFactor)");
 ...
 case 'save':
	...
	$txtFactor=getVal($_POST['txtFactor']);
	if(hasVal($name)&&hasVal($txtFactor)){
		$sql_update = db_query("UPDATE `deptos` SET `nombre`=$name,factor=$txtFactor,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Departamento</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					<tr>
						...
						<td class="input"><?php echo getInputField('txtNombre','',"maxlength='50'");?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Factor:</span></td>
						<td class="input"><?php echo getInputField('txtFactor','0.00',"maxlength='4'");?></td>
					</tr>
				</table>
				<br class="clear"/>
 ...
 <script type="text/javascript" src="js/val/deptos.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `deptos` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Departamento</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Nombre:</span></td>
						<td class="input"><?php echo getInputDisable('txtNombre',htmlspecialchars($nombre),"maxlength='50'");?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Factor:</span></td>
						<td class="input"><?php echo getInputDisable('txtFactor',htmlspecialchars($factor),"maxlength='4'");?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
...
default:
	...
	if($_POST['delState']){
		...
		$sql = "SELECT id, nombre, factor,active FROM deptos ".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("id"=>"ID","nombre"=>"Nombre","factot"=>"Factor","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Departamentos</h2></div>
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					...
					echo "<td>".htmlspecialchars($row[factor])."</td>\n";
