...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name) && hasVal($idregion)){
		$sql_update = db_query("INSERT INTO `comuna` (`nombre`, `idpoligono`) VALUES ($name, $idregion)");
 ...
 case 'save':
	...
	if(hasVal($name)){
		$sql_update = db_query("UPDATE `comuna` SET `nombre` = $name, idpoligono=$idregion, `modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar municipio</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Poligono:</span></td>
						<td class="input"><?php echo getComboAdjust("SELECT id,nombre,active FROM poligono",'txtRegion');?></td>
					</tr>
				</table>
				<br class="clear"/>
 ...
 <script type="text/javascript" src="js/val/gestion_comuna.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `comuna` WHERE `id` = '$id'");
	$row = mysqli_fetch_array($r);
	if (count($row)>0) {
		$name = $row['nombre'];
		$idregion = $row['idpoligono'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar municipio</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Poligono:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM poligono",'txtRegion',htmlspecialchars($idregion));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `comuna` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `comuna` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `comuna` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT a.id, a.nombre, b.nombre poligono, a.active, a.create_date 
				FROM comuna a 
				JOIN poligono b
				ON a.idpoligono = b.id
				WHERE 1=1 ".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("id"=>"ID","a.nombre"=>"Nombre municipio","b.nombre"=>"Poligono","create_date"=>"Fecha creacion","active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Municipio</h2></div>
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					...
					echo "<td>".htmlspecialchars($row['poligono'])."</td>\n";
