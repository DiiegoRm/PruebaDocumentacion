...
switch($_REQUEST["mode"]){
 case 'new':
	...
	$idregion=getVal($_POST['txtRegion']);
	$idjefe=getVal($_POST['txtJefe'],"null");
	if(hasVal($name)&&hasVal($idregion)&&hasVal($idjefe)){
		$sql_update = db_query("INSERT INTO `jefaturas` (`nombre`,`idregion`,`idjefe`) VALUES ($name,$idregion,$idjefe)");
 ...
 case 'save':
	...
	if(hasVal($name)&&hasVal($idregion)&&hasVal($idjefe)){
		$sql_update = db_query("UPDATE `jefaturas` SET `nombre`=$name,idregion=$idregion,idjefe=$idjefe,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Jefatura</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						...
						<td class="input"><?php echo getComboAdjust("SELECT id,nombre,active FROM regiones",'txtRegion');?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Jefe:</span></td>
						<td class="input"><?php echo getComboAdjust("SELECT id,nombre,active FROM jefes",'txtJefe');?></td>
					</tr>
				</table>
				<br class="clear"/>
 ...
 <script type="text/javascript" src="js/val/jefaturas.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `jefaturas` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Jefatura</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Region:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM regiones",'txtRegion',htmlspecialchars($idregion));?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Jefe:</span></td>
						<td class="input"><?php echo getComboAdjustDisable("SELECT id,nombre,active FROM jefes",'txtJefe',htmlspecialchars($idjefe));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
 ...
 <script type="text/javascript" src="js/val/jefatuaras.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
	 }
 break;
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `jefaturas` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `jefaturas` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `jefaturas` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT j.id,j.nombre,j.active,f.nombre jefe, r.nombre region FROM jefaturas j LEFT JOIN jefes f ON j.idjefe = f.id JOIN regiones r ON j.idregion=r.id".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("j.id"=>"ID","j.nombre"=>"Nombre Jefatura","f.nombre"=>"Jefe","r.nombre"=>"Region","j.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Jefaturas Comerciales</h2></div>
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					...
					echo "<td>".htmlspecialchars($row[jefe])."</td>\n";
