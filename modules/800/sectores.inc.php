...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name)&&hasVal($idlocalidad)){
		$sql_update = db_query("INSERT INTO `sectores` (`nombre`,`idlocalidad`) VALUES ($name,$idlocalidad)");
 ...
 case 'save':
	...
	if(hasVal($name)&&hasVal($idlocalidad)){
		$sql_update = db_query("UPDATE `sectores` SET `nombre`=$name,idlocalidad=$idlocalidad,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
...
<div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Sector</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						...
						<td class="field"><?php echo getComboBox("SELECT id,nombre,active FROM localidades",'txtLocalidad');?></td>
					</tr>
				</table>
				<br class="clear"/>
...
<?php
 ...
 case 'edit':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `sectores` WHERE `id` = '$id'");
...
<script type="text/javascript" src="js/ui/cb.localidad.js?ver=<?php echo SGP_VERSION?>"></script>
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Sector</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Localidad:</span></td>
						<td class="field"><?php echo getComboDisable("SELECT id,nombre,active FROM localidades",'txtLocalidad',htmlspecialchars($idlocalidad));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
...
default:
	...
	if($_POST['delState']){
		...
		$sql = "SELECT s.id,s.nombre,s.active,l.nombre localidad FROM sectores s, localidades l WHERE s.idlocalidad=l.id".getSQLFilters().getSQLSort();
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("s.id"=>"ID","s.nombre"=>"Sector","l.nombre"=>"Localidad","s.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Sectores</h2></div>
