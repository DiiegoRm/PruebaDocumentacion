...
switch($_REQUEST["mode"]){
 case 'new':
	...
	$code=getStrVal($_POST['txtCodigo']);
	$distribuidor=getVal($_POST['txtLocalidad']);
	if(hasVal($name)&&hasVal($code)&&hasVal($distribuidor)){
		$sql_update = db_query("INSERT INTO `armarios` (`codigo`,`central`,`idDistribuidor`) VALUES ($name,$code,$distribuidor)");
 ...
 case 'save':
	...
	$distribuidor=getVal($_POST['txtLocalidad']);
	if(hasVal($name)&&hasVal($code)&&hasVal($distribuidor)){
		$sql_update = db_query("UPDATE `armarios` SET `codigo`=$name,`central`=$code,idDistribuidor=$distribuidor,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Armario</h2></div>
			...
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=new">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Central:</span></td>
						<td class="field"><?php echo getInputField('txtCodigo');?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Distribuidor:</span></td>
						<td class="field"><?php echo getComboBox1("SELECT codigo, CONCAT(codigo,' | ',nombre) nombre, active FROM distribuidores",'txtLocalidad');?></td>
					</tr>
				</table>
 ...
 <script type="text/javascript" src="js/val/distribuidores.js?ver=<?php echo SGP_VERSION?>"></script>
<?php
 break;
 case 'edit':

	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `armarios` WHERE `id` = '$id'");
	$row = mysqli_fetch_array($r);
	if (count($row)>0) {
		$name = $row['codigo'];
		$code = $row['central'];
		$distribuidor = $row['idDistribuidor'];
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Armario</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
			<form name="frmSubmit" id="frmSubmit" method="post" action="?menu=<?php echo getMenu();?>&amp;mode=save">
				<table class="data-ro" id="tables-all" style="width:50%">
					...
					<tr>
						<td class="title"><span class="required">*</span>Central:</span></td>
						<td class="field"><?php echo getInputDisable('txtCodigo',htmlspecialchars($code));?></td>
					</tr>
					<tr>
						<td class="title"><span class="required">*</span>Distriibuidor:</span></td>
						<td class="field"><?php echo getComboDisable1("SELECT codigo, CONCAT(codigo,' | ',nombre) nombre, active FROM distribuidores",'txtLocalidad',htmlspecialchars($distribuidor));?></td>
					</tr>
				</table>
				<br class="clear"/>
				<div class="formbuttons">
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `armarios` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `armarios` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `armarios` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT a.id,a.codigo,d.codigo as codis,d.nombre as distribuidor,a.central as central,a.active FROM armarios a,distribuidores d WHERE a.idDistribuidor=d.codigo $variable".getSQLFilters().getSQLSort("a.id","ASC");
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("a.id"=>"ID","a.codigo"=>"Armario","d.nombre"=>"Distribuidor","d.codigo"=>"Cod. Distribuidor","a.central"=>"Central","a.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Armarios</h2></div>
		...
		<div class="actionbar">
			<?php printButtonSetarm($appuser,$fields) ?>
		</div>
		<div>
...
<?php
				...
				while($row = mysqli_fetch_array($query)) {
					...
					echo "<td>".htmlspecialchars($row[distribuidor])."</td>\n";
					echo "<td>".htmlspecialchars($row[codis])."</td>\n";
					echo "<td>".htmlspecialchars($row[central])."</td>\n";
