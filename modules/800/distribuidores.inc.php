...
switch($_REQUEST["mode"]){
 case 'new':
	...
	if(hasVal($name)&&hasVal($code)&&hasVal($idlocalidad)){
		$sql_update = db_query("INSERT INTO `distribuidores` (`nombre`,`codigo`,`idlocalidad`) VALUES ($name,$code,$idlocalidad)");
 ...
 case 'save':
	...
	$idlocalidad=getVal($_POST['txtLocalidad']);
	if(hasVal($name)&&hasVal($code)&&hasVal($idlocalidad)){
		$sql_update = db_query("UPDATE `distribuidores` SET `nombre`=$name,`codigo`=$code,idlocalidad=$idlocalidad,`modify_date`=CURRENT_TIMESTAMP WHERE `id`=$id");
		printMessage("Actualizando base de datos, por favor espere..","ok");
	}
	else {
		printMessage("No ha completado los campos obligatorios...","error");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Adicionar Distribuidor</h2></div>
...
<?php
 ...
 case 'edit':
	...
	$id=getVal($_GET['id']);
	$r =  db_query("SELECT * FROM `distribuidores` WHERE `id` = '$id'");
 ...
 <div class="section">
	<div class="info">
	 <div class="formpage">
		<div class="outerbox">
			<div class="mainHeading"><h2>Editar Distribuidor</h2></div>
			 <div class="messagebar">
                <span id="message" class="error"></span>
            </div>
	...
	if($_POST['delState']){
		...
		for ($i=0; $i < $n; $i++){
			switch($_POST['delState']){
				case 'DeleteMode':
					$sql_update = db_query("DELETE FROM `distribuidores` WHERE id={$del[$i]}");
					break;
				case 'EnableMode':
					$sql_update = db_query("UPDATE `distribuidores` SET `active`='Si' WHERE id={$del[$i]}");
					break;
				case 'DisableMode':
					$sql_update = db_query("UPDATE `distribuidores` SET `active`='No' WHERE id={$del[$i]}");
					break;
			}
		}
		printMessage("Actualizando base de datos, por favor espere..","ok");
		...
		$sql = "SELECT di.id,di.codigo,di.nombre,di.active,l.nombre as localidad,d.nombre depto FROM distribuidores di, localidades l,deptos d WHERE di.idlocalidad=l.id AND l.iddepto=d.id".getSQLFilters().getSQLSort("di.id","ASC");
		$q = db_query($sql);
		$regCount = mysqli_num_rows($q);

		$maxPage = ceil($regCount/$rowsxPage);
		$rowFrom = (($pageNO-1) * $rowsxPage);
		$fields = array("di.id"=>"ID","di.codigo"=>"Codigo","di.nombre"=>"Nombre","l.nombre"=>"Localidad","d.nombre"=>"Departamento","di.active"=>"Activo");
?>
<div class="section">
	<div class="info">
	 <div class="outerbox">
		<div class="mainHeading"><h2>Distribuidores</h2></div>
