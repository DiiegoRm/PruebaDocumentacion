$(document).ready(function(){
	...
	let labelTxtAuditado = $("#labelTxtAuditado");
	...
	txtEstado.change(validarNotificacion);
	txtHallazgo.change(validarNotificacion);
	txtCertificado.change(validarNotificacion);
	labelTxtAuditado.hide();


	form.submit(function(){
		if(valtxtEstado() && valtxtCertificado() && valtxtObservaciones() && valtxtHallazgo() && valtxtAuditado() && valtxtRepresentante()) {
			return true;
		}
		else{
	...
	function valtxtObservaciones(){
		...
		if(txtObservaciones.val() == ""){
			txtObservaciones.css({"border":"1px solid #e77776"});
			return false;
		}
		//it's valid
		else{			
			txtObservaciones.css({"border":"1px solid #c5dbec"});
			return true;
		}
	}
	...
	/* Desarollo auditorias Validacion de la notifcacion*/
	function validarNotificacion(){
		if(txtEstado.val() == 'Malo' ||  txtCertificado.val() == 'No Cumple' || txtHallazgo.val() == 'Si'){
			labelTxtAuditado.show();
			return true;
		}
		labelTxtAuditado.hide();
